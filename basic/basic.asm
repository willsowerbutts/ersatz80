...
;; WRS - console i/o routines
        ...
        .ORG    00010H          ; input character to A; wait until ready
        JP ICWAIT

        .ORG    00018H          ; check input ready status
        IN A, ($00)             ; read UART status
        BIT 7, A                ; perform the test
        RET                     ; return with flags set

OUTCHAR:PUSH AF
OCWAIT: IN A, ($00)             ; read UART status
        BIT 6, A                ; test status bit
        JR NZ, OCWAIT           ; wait for UART TX ready
        POP AF
        OUT ($01), A            ; output character
        RET

ICWAIT: IN A, ($00)             ; read UART status
        BIT 7, A                ; test top bit
        JR Z, ICWAIT            ; loop if no char waiting
        IN A, ($01)             ; read UART data
        RET
